...
def sync_customers_window(days_back: int):
    ...
    url = (f"{PRI_BASE}/CUSTOMERS?$filter=CREATEDDATE%20ge%20{lo}T00:00:00%2B02:00"
           # CELLPHONE אינו שדה של CUSTOMERS — ה-400 שהוא גרם הקפיא את סנכרון הלקוחות
           # מ-17.9 עד 20.9.2026 (bi_customers עצר ב-16.9). הטלפון הנוסף הוא SPEC2.
           "&$select=CUSTNAME,CITYNAME,RONY_SUGCUSTDES,SPEC4,CREATEDDATE,PHONE,SPEC2")
    rows = [{"cn": r.get("CUSTNAME") or "", "ct": r.get("CITYNAME") or "",
             "sc": r.get("RONY_SUGCUSTDES") or "", "sr": r.get("SPEC4") or "",
             "cr": (r.get("CREATEDDATE") or "")[:10],
             "ph": [x for x in (r.get("PHONE"), r.get("SPEC2")) if x]}
...
kind — בדיוק אחד:
- thanks — רק כשיש שבח מפורש או הבעת שביעות רצון: "הספה מדהימה", "שירות מעולה", "מרוצים מאוד", "הגיעו בדיוק בזמן", "המוביל היה מקצועי". "תודה" / "תודה רבה" / "מעולה תודה" לבד, בלי תוכן — זה נימוס, לא thanks: סווג neutral. תודה בסוף בקשה או תלונה — neutral.
...
q — ציטוט מילולי מהלקוח, המשפט המייצג ביותר, עד 100 תווים, בלי טלפונים ובלי שמות של הלקוח או בני משפחתו (שמות של עובדי החנות ומובילים מותר — חשוב לדעת מי זכה לשבח). ריק אם ניטרלי.
conf — ביטחון 0 עד 1.
...
# ערך הסינון שמסך המוצרים שולח כשבוחרים "ספות בלבד" — לא שם של משפחה בפריוריטי.
SOFAS_FAM = "__sofas__"
...
def api_products(request: Request, d_from: str = "", d_to: str = "",
    ...
    # "מה נחשב ספה" עבר ל-SQL (bi_is_sofa) — כלל על כל מק"ט בנפרד. הכרעת דורון
    # 24.9.2026: כל כריות המחולל בפנים (נעשו בבד הספה), "נוי" יוצא רק בקטגוריית
    # "כריות" הרגילה. קריאה אחת במקום אחת לכל משפחה, ואותו כלל לכל מסך שישאל.
...
@app.get("/api/hygiene")
def api_hygiene(request: Request):
    """Zero-entry days that the weekly check could not explain.

    The screen shows a footfall-based conversion rate, so a day the counter
    missed is not a quiet gap - it is a denominator that never existed, silently
    dragging that day's rate toward zero. This is what lets the screen say so.

    Answers with what was checked and when, not only with findings, so an empty
    list reads as "checked, nothing open" rather than as silence.
    """
    if not _logged_in(request):
        return JSONResponse({"error": "auth"}, status_code=401)
    try:
