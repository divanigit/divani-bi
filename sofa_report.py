...
# "מה נחשב ספה" יושב ב-SQL — public.bi_is_sofa — ולא כאן, כי זה כלל על כל
# מק"ט בנפרד ורשימת קטגוריות לא יכולה לבטא אותו. הכרעת דורון 24.9.2026
# (מחליפה את 9.9): כל מה שבקטגוריות המחולל הוא חלק מהספה, כולל כרית שכתוב
# עליה "נוי" — היא נעשתה בהזמנה בבד הספה. "נוי" יוצא רק בקטגוריית "כריות"
# הרגילה (כרית נוי-תצוגה, כרית נוי בעיצוב אישי). הגרסה שלפני 9.9 סיווגה לפי
# שם קטגוריה, ו"כרית" ביחיד לא תפס את "כריות" ברבים. מקור הגירסאות:
# cloud\sql\bi_products.sql.
...
def other_families(families, sofa_fams):
    """הקטגוריות שלא נכנסו ל"ספות בלבד". קטגוריית "כריות" הרגילה חוזרת מהענן עם התוספת
